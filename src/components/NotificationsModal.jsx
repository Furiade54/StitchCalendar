...
const NotificationsModal = ({ isOpen, onClose, notifications, onRespond, onMarkRead }) => {
              ...
              >
                <div className="flex items-start gap-3">
                  <div className={`size-10 rounded-full flex items-center justify-center shrink-0 ${
                    notification.type === 'family_request' ? 'bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400' :
                    notification.type === 'editor_grant' ? 'bg-green-100 dark:bg-green-900/30 text-green-600 dark:text-green-400' :
                    'bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-400'
                  }`}>
                    <span className="material-symbols-outlined">
                      {notification.type === 'family_request' ? 'group_add' :
                       notification.type === 'editor_grant' ? 'how_to_reg' :
                       'block'}
                    </span>
                  </div>
                  <div className="flex-1">
                    <h4 className="font-bold text-slate-800 dark:text-slate-200 text-sm">
                      {notification.type === 'family_request' ? (notification.status === 'accepted' ? 'Solicitud aceptada' : notification.status === 'rejected' ? 'Solicitud rechazada' : 'Solicitud de Grupo Familiar') :
                       notification.type === 'editor_grant' ? 'Permiso de Edición Concedido' :
                       'Permiso de Edición Revocado'}
                    </h4>
                    <p className="text-sm text-slate-600 dark:text-slate-400 mt-1">
                      {notification.type === 'family_request' ? (
                        <>
                          {notification.status === 'accepted' ? (
                            <>
                              Aceptaste la invitación de <span className="font-semibold text-primary">{notification.fromUserName}</span> para unirte a su grupo familiar.
                            </>
                          ) : notification.status === 'rejected' ? (
                            <>
                              Rechazaste la invitación de <span className="font-semibold text-primary">{notification.fromUserName}</span>.
                            </>
                          ) : (
                            <>
                      ...
                      ) : notification.type === 'editor_grant' ? (
                        <>
                          <span className="font-semibold text-primary">{notification.fromUserName}</span> te otorgó acceso para editar su calendario.
                        </>
                      ) : (
                        <>
                          <span className="font-semibold text-primary">{notification.fromUserName}</span> revocó tu acceso para editar su calendario.
                        </>
                      )}
                    </p>
                ...
                {notification.type === 'family_request' && notification.status === 'pending' && (
                ...
                {notification.type === 'family_request' && (notification.status === 'accepted' || notification.status === 'rejected') && onMarkRead && (
                  <div className="flex items-center gap-2 mt-2">
                    <button
                      onClick={() => onMarkRead(notification.id)}
                      className="w-full py-2 px-4 rounded-xl bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 font-semibold text-sm hover:bg-slate-200 dark:hover:bg-slate-600 transition-colors"
                    >
                      Marcar como leída
                    </button>
                  </div>
